...
helper_path = Path(sys.argv[4]) #'/ix/rbao/Projects/panCancer_HE/scripts/pancancer_he_classifier'
...
use_stain_norm = True
ignore_artifacts = ignore_problems #Found to be an issue in some datasets like acral melanoma
...
if file_avail:    
    ...
    if 'btf' in file_type:
        wsi = HESlide(str(fn), backend='BioFormats') #I think this runs slower than default!
    else:
        wsi = HESlide(str(fn))
                        ...
                        if (per > tissue_thresh) :
                            ...
                            if use_anno:
                                ...
                                if max_overlap > gj_anno_overlap:
                                    ...
                                    tile_df.loc[ii,'anno'] = 'notAnnotated' #Prevously blank
