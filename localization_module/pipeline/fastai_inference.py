...
V11 -- create model tag from model filename
"""
...
pos_cls_col = int(sys.argv[5])
model_tag = model_fn.parts[-1].split(".")[0]  # TODO: implement model labeling
s = time.time()
...
learn = load_learner(
    model_fn
)  # Run with GPU to get time improvement? : learn = load_learner(model_fn, cpu = False)
...
df.loc[:, "p_pos"] = p[:, pos_cls_col]  # ['Tumor', 'notTumor'] (via dls.vocab)
df.loc[:, "pred_cls"] = c
df.loc[:, "pos_cls"] = pos_cls_col
