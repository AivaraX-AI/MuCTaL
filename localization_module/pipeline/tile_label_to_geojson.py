...
for label in tqdm(tile.loc[:,label_col].unique()):
    ...
    if method == 'default':
        for x,y in zip(xx,yy):
            roi[(x-h_ds_ts+1):(x+h_ds_ts-1),
                (y-h_ds_ts+1):(y+h_ds_ts-1)] = 255
    ...
    roi = roi.T
    local_mean = gaussian_filter(roi, sigma=1) #very limited smoothing
    im = cv2.normalize(src=local_mean, dst=None, alpha=0, 
                       beta=255, norm_type=cv2.NORM_MINMAX, 
                       dtype=cv2.CV_8UC1)
    _, im = cv2.threshold(im, 20, 255, cv2.THRESH_BINARY)
    geocontours = find_geocontours(im, mode='imagej')
    features = [contour.export_feature(color=(0, 255, 0), 
                                       label=label) for contour in geocontours]
...
fn = output.joinpath("%s_ds5_10sm_tile_offset_112px_%s_labeling.geojson" % (slide_name,label_col))
print(fn)
