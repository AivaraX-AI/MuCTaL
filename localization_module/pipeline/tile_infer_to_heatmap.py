...
if len(sys.argv) == 4:
    ...
    method = sys.argv[
        4
    ]  # 'anno_overlap' is for showing how geojson overlaps with image
...
if bkg_fn.exists():
    ...
    if method == "infer":
        ...
        save_fn = heatmap_path.joinpath(
            "%s_tile_training_annotation_overlap_heatmap_ds%dx.png" % (base_file, ds)
        )
    print("make:", save_fn)
    ...
    if method == "infer":
        comb_fn = heatmap_path.joinpath(
            "%s_pred_tumor_overlay_ds%dx.jpg" % (base_file, ds)
        )
    elif method == "anno_overlap":
        comb_fn = heatmap_path.joinpath(
            "%s_training_annotation_overlay_ds%dx.jpg" % (base_file, ds)
        )
    print(comb_fn)
