...
if len(sys.argv) == 4:
    ...
    method = sys.argv[4] # 'anno_overlap' is for showing how geojson overlaps with image
print('Heatmap method: %s' % method)
...
if bkg_fn.exists():
    ...
    if method == 'infer':
        ...
        save_fn = heatmap_path.joinpath('%s_tile_heatmap_ds%dx.png' % (base_file, ds))
    elif method == 'anno_overlap':
        tile_fns = df.loc[:,'tile'].values
        use_p = df.loc[:,'overlap'].values / 100
        save_fn = heatmap_path.joinpath('%s_tile_training_annotation_overlap_heatmap_ds%dx.png' % (base_file, ds))
    print('make:', save_fn)
    ...
    comb = cv2.addWeighted(bkg,0.6,
                           heat_map,0.4, 30)
    if method == 'infer':
        comb_fn = heatmap_path.joinpath('%s_pred_tumor_overlay_ds%dx.jpg' % (base_file,ds))
    elif method == 'anno_overlap':
        comb_fn = heatmap_path.joinpath('%s_training_annotation_overlay_ds%dx.jpg' % (base_file,ds))
    print(comb_fn)
