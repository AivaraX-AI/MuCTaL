...
def plot_geojson_feature(feat,
                         ax,
                         class_cm = {'Malignant':'r',
                                     'Benign Bile Duct':'b'}):
...
def index_tiles_of_core_with_feature(feat,subset,tma_dat, rotate_180=False):
    ...
    if rotate_180:
        ...
        core_width = tma_dat.loc[
                        tma_dat.loc[:,'Core #']== core,
                        'Width'
                          ].values
    ...
    if rotate_180:
        idx0 = (fx > (core_width - subset.norm_x)) & (fx <= (core_width - subset.norm_x + subset.width))
        idx1 = (fy > (core_width - subset.norm_y)) & (fy <= (core_width - subset.norm_y + subset.width))
    else:
...
def calc_percent_poscells_in_tumor_tiles(allobjects,
                                         subset,
                                         tma_dat,
                                         ax=None,
                                         use_plot=False,
                                         class_cm = {'Positive':'yellow',
                                                     'Negative':'cyan'},
    ...
    for feat in allobjects:
        anno_class = feat['properties']['classification']['name']
        if anno_class in anno_class_cm:
            idx = index_tiles_of_core_with_feature(feat, 
                                                   subset,
                                                   tma_dat,
...
def check_tile_near_feature(feat,tile_xy,tile_size):
    ...
    #Check if any corner is in an annotation:
    points = [tile_xy,
               [tile_xy[0] + tile_size,tile_xy[1]+tile_size],
              [tile_xy[0] + tile_size,tile_xy[1]],
              [tile_xy[0], tile_xy[1]+tile_size]]
              
    out = check_points_in_feature(feat,points) 
    nearby = np.any(out)          
    return nearby
...
def check_tile_overlap_feat(feat,tile_xy,tile_size):
    points=[]
    per_overlap = 0
    nearby = check_tile_near_feature(feat,tile_xy,tile_size)
    if nearby:
...
def add_coords_to_tile_df(tile_df,
                          swap_xy = True):
    ...
    if swap_xy: #True for now
        tile_df.loc[:,'y']= tile_df.cur_path.str.split('_').str[4].str[1:].astype(int) #Reversed x & y
        tile_df.loc[:,'x']= tile_df.cur_path.str.split('_').str[5].str[1:].astype(int)
    else:
        tile_df.loc[:,'x']= tile_df.cur_path.str.split('_').str[4].str[1:].astype(int) #Reversed x & y
        tile_df.loc[:,'y']=tile_df.cur_path.str.split('_').str[5].str[1:].astype(int)
        
    tile_df.loc[:,'width']=tile_df.cur_path.str.split('_').str[6].str[2:].str.split('.').str[0].astype(int)
                                                                                          
    return tile_df

def unify_core_numbers(df, 
                       use_col ='Core name',
                       out_col = 'Core #'):
