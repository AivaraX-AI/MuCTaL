...
def sample_from_different_slides(n_samples, groups, random_state=None):
    ...
    if len(unique_groups) < n_samples:
        print(f"Warning not enough groups ({len(unique_groups)}) to sample {n_samples} examples.")
        replace = True
    else:
        replace = False
...
def plot_tile_grid(data,
    ...
    for i,cluster in enumerate(clusters):
        ...
        fig, axes = plt.subplots(nrows= grid_size[0], 
                                ncols=grid_size[1], 
                                layout = "constrained",
                                figsize=figsize) 
        ...
        while (len(use) < n_samples):
            exs = all_ex[sample_from_different_slides(n_samples, adata_obs.loc[all_ex,'slide'])]
            j = 0
