...
if fold_method == "crossvalid":  # Otherwise include all data for test (see below)
    ...
    if "fold" in crossval_method:  # Default for most models
        ...
        balance_tiles_per_target = [
            True,
            True,
        ]  # Train, Test -- balance n tiles drawn from each target
        n_repeats = (
    ...
    for target_train_idx, target_test_idx in splitter.split(
        target_df.loc[:, bal_target],
        target_df.loc[:, "group"],
        ...
        if "train" in crossval_method:
            ...
            if (np.sum(target_df.loc[target_train_idx, "anno"] == True) == k // 2) and (
                np.sum(target_df.loc[target_train_idx, "anno"] == False) == k // 2
            ):
    ...
    for target_train_idx, target_test_idx in zip(ktrain, ktest):
        ...
        df_list = []
        n_train_test = [
            np.sum(target_df.loc[target_train_idx, "anno"] == True),  # n pos
            np.sum(target_df.loc[target_train_idx, "anno"] == False),  # n neg
            np.sum(target_df.loc[target_test_idx, "anno"] == True),  # n pos
            np.sum(target_df.loc[target_test_idx, "anno"] == False),
        ]  # n neg
        print(
            "Train targets n=%d pos, %d neg, Validation targets n=%d pos, %d neg"
            % tuple(n_train_test)
        )
        ...
        for i, ds in enumerate(sets):
            ...
            if balance_tiles_per_target[i]:
                ...
                for ii, target_id in enumerate(use_cases):
                    keep = np.argwhere(
                        np.array(all_dat.loc[:, "case"].isin([target_id]))
                    ).flatten()
                    n0 = keep.shape[0]
                    on.append(n0)
                    if n0 > ntarget:
                ...
                use_tiles = all_dat.loc[:, "case"].isin(
                    use_cases
                )  # Index of tiles from target(s) in current set 'ds' (train / valid)
                bal_dat = pd.DataFrame(
                    all_dat.loc[use_tiles, ("fn", bal_target, "case", "anno")]
                )
            ...
            if balance_classes_total_tiles[i]:
                ...
                if (
                    use_n == 0
                ):  # If there are no targets in one class, revert to n of class with targets!
                ...
                temp = pd.DataFrame(
                    bal_dat.loc[bal_tiles_idx[:, 0], ("fn", bal_target, "case", "anno")]
                ).reset_index(drop=True)
            else:
                ...
                temp = pd.DataFrame(
                    bal_dat.loc[:, ("fn", bal_target, "case", "anno")]
                ).reset_index(drop=True)
            temp["is_valid"] = i
            if i == 0:
                out = temp
