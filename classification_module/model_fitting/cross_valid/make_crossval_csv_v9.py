'''
    ...
    #Version 9 - use cases as opposed to slide for creating folds, replace repeated kfold with StratifiedGroupKFold
'''
...
split_data_path = Path(sys.argv[1])  # v{n}/tiles/'224px' or blah/'500px' etc. but will assume [vn]/parse/[type] for version and type
...
if fold_method == 'crossvalid':  # Otherwise include all data for test (see below)
    ...
    for slide_train_idx, slide_test_idx in zip(ktrain, ktest):
        ...
        for i, ds in enumerate(sets):
            ...
            if balance_tiles_per_slide[i]:
                ...
                for ii, slide_id in enumerate(use_cases):
                    ...
                    if n0 > ntarget:
                        keep = resample(keep,
                                        ...
                                        replace=False, )
                    if ii == 0:
                        bal_dat = all_dat.loc[keep, ('fn', 'slide','case', 'class')].reset_index(drop=True)
                    else:
                        temp = all_dat.loc[keep, ('fn', 'slide','case', 'class')].reset_index(drop=True)
                ...
                bal_dat = pd.DataFrame(all_dat.loc[use_tiles, ('fn', 'slide','case', 'class')])
            ...
            if balance_classes_total_tiles[i]:
                ...
                use_class = bal_dat.loc[:, 'class'].str.contains(pos_class)
                ...
                use_class = bal_dat.loc[:, 'class'].str.contains(pos_class).values
                npos = np.sum(use_class)
                nneg = len(use_class) - npos
                print('Not balancing class totals for %s ... %d neg %d pos, use all.'
                      % (set_lab[i], nneg, npos))
                temp = pd.DataFrame(bal_dat.loc[:, ('fn', 'slide', 'case','class')]
                                    ).reset_index(drop=True)
            temp['is_valid'] = i
            if i == 0:
                out = temp
        ...
        print('if nbal > 0, n_pos =',
              np.sum(np.array(out.loc[out.loc[:, 'is_valid'] == 0, 'class']) == pos_class),
              '(should ==) n_neg=',
              np.sum(np.array(out.loc[out.loc[:, 'is_valid'] == 0, 'class']) != pos_class)
              )
    ...
    fold = 0
    out = all_dat.loc[:, ('fn', 'slide','case', 'class')]
