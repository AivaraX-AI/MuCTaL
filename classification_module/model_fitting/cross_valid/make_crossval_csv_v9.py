...
split_data_path = Path(
    sys.argv[1]
)  # v{n}/tiles/'224px' or blah/'500px' etc. but will assume [vn]/parse/[type] for version and type
...
if fold_method == "crossvalid":  # Otherwise include all data for test (see below)
    ...
    for slide_train_idx, slide_test_idx in splitter.split(
        ...
        if "train" in crossval_method:
            ...
            if (np.sum(slide_df.loc[slide_train_idx, "class"] == True) == k // 2) and (
                np.sum(slide_df.loc[slide_train_idx, "class"] == False) == k // 2
            ):
    ...
    for slide_train_idx, slide_test_idx in zip(ktrain, ktest):
        ...
        for i, ds in enumerate(sets):
            ...
            if balance_tiles_per_slide[i]:
                ...
                bal_dat = pd.DataFrame(
                    all_dat.loc[use_tiles, ("fn", "slide", "case", "class")]
                )
            ...
            if balance_classes_total_tiles[i]:
                ...
                temp = pd.DataFrame(
                    bal_dat.loc[bal_tiles_idx[:, 0], ("fn", "slide", "case", "class")]
                ).reset_index(drop=True)
            else:
                ...
                temp = pd.DataFrame(
                    bal_dat.loc[:, ("fn", "slide", "case", "class")]
                ).reset_index(drop=True)
            temp["is_valid"] = i
            if i == 0:
                out = temp
        ...
        print(
            "if nbal > 0, n_pos =",
            np.sum(
                np.array(out.loc[out.loc[:, "is_valid"] == 0, "class"]) == pos_class
            ),
            "(should ==) n_neg=",
            np.sum(
                np.array(out.loc[out.loc[:, "is_valid"] == 0, "class"]) != pos_class
