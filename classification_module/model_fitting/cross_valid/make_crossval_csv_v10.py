'''
    ...
    inputs: $1 - split_data_path, str, path to use for train/validation spreadsheet(s)
            ...
            $2 - use_model, str describing model and cross-validation to perform
                e.g. 'resnet18_10fold_10rep_500bal_224px'
                     arch_cvmethod_nreps_nbalance_nft -> parsed as parameters for CV
                    -architecture = 'resnet18'
                    -cv method = '10fold' k-fold cross validation (10 is parsed),
                                '2train' (4,8,..n train) limits training to tiles
                                 drawn from n training slides (digit is parsed)
                                'jackknife' performs 'LeaveOneGroupOut' CV
                    -repetitions = '10rep' number of repetitions of kfold using different seeds
                                  (digit is parsed).
                    -balancing = '500bal' - resampling to balance class examples of tiles
                                 per slide during training
                                 (digit is parsed)
                    -ft = '2ft' --> used to indicate finetuning amount (no longer used)
                    -px = number of pixels to use in model
                use_model also used as output directory for .csv files:
                    $results/v$n/models/224px/$use_model/csv
            $3 - fold_method, str, flag for how to interpret the split_data_path
                - 'crossvalid' - use all parameters as described above in use_model code
                - 'newdata' - assume use_model describes previously fit model(s) and
                             so ignore them, and make a single .csv file in new data path with
                             kfold*nrep sym links (one for each previously trained model)
                             for compatibility between cross-validation and new-data inference.
           $4 - slide_df_fn - str
           $5 - all_dat_fn - str
    outputs:
        kfold * nrep .csv files describing which tiles to use for training and validating for each
        split of the data, saved to $split_data_path.parent/['tiles_model']/$use_model/csv
        ver = 8 #Now uses dataframes that list tiles to get class information 
          -class name is no longer stored in tile filename.
          - tile set may contain multiple file types (tif jpeg jpg png etc)
    ver = 9 - use cases as opposed to target for creating folds, replace repeated kfold with StratifiedGroupKFold
    ver = 10- switch tiles 'class' to 'anno'
            - remove : split_data_path = Path(sys.argv[1]) and renumber
...
tile_version = ver
tile_type = '224px'
use_model = sys.argv[1]  # i.e. "resnet18_jackknife", "resnet18_10fold_10rep_500bal_5ft", resnet18_2train_100rep_25bal  
...
if fold_method == 'crossvalid':  # Otherwise include all data for test (see below)
    ...
    if nbal == 0:
        ...
        balance_tiles_per_slide = [False, False]  # Train, Test - whether to balance to nbal    
    else:
        # How to balance training example tiles during training and testing:
        balance_classes_total_tiles = [True, True]  # Train, Test - whether to balance to nbal
        balance_tiles_per_slide = [True, True]  # Train, Test - whether to balance to nbal
    ...
    if 'fold' in crossval_method: #Default for most models
        ...
        balance_tiles_per_slide = [True, True]  # Train, Test -- balance n tiles drawn from each slide
    ...
    for slide_train_idx, slide_test_idx in splitter.split(slide_df.loc[:, 'slide'],
        ...
        if 'train' in crossval_method:
            ...
            if (np.sum(slide_df.loc[slide_train_idx, 'anno'] == True) == k // 2) \
                    and (np.sum(slide_df.loc[slide_train_idx, 'anno'] == False) == k // 2):
    ...
    for slide_train_idx, slide_test_idx in zip(ktrain, ktest):
        ...
        set_lab = ['train', 'valid']
        df_list = []
        n_train_test = [np.sum(slide_df.loc[slide_train_idx, 'anno'] == True),  # n pos
                        np.sum(slide_df.loc[slide_train_idx, 'anno'] == False),  # n neg
                        np.sum(slide_df.loc[slide_test_idx, 'anno'] == True),  # n pos
                        np.sum(slide_df.loc[slide_test_idx, 'anno'] == False)]  # n neg
        print('Train slides n=%d pos, %d neg, Validation slides n=%d pos, %d neg' % tuple(n_train_test))
        ...
        for i, ds in enumerate(sets):
            ...
            if balance_tiles_per_slide[i]:
                ...
                for ii, slide_id in enumerate(use_cases):
                    keep = np.argwhere(np.array(all_dat.loc[:, 'case'].isin([slide_id]))
                                       ).flatten()
                    n0 = keep.shape[0]
                    on.append(n0)
                    if n0 > ntarget:
                        keep = resample(keep,
                                        ...
                                        replace=False, )
                    if ii == 0:
                        bal_dat = all_dat.loc[keep, ('fn', 'slide','case', 'anno')].reset_index(drop=True)
                    else:
                        temp = all_dat.loc[keep, ('fn', 'slide','case', 'anno')].reset_index(drop=True)
                ...
                use_tiles = all_dat.loc[:, 'case'].isin(
                    use_cases)  # Index of tiles from slide(s) in current set 'ds' (train / valid)
                bal_dat = pd.DataFrame(all_dat.loc[use_tiles, ('fn', 'slide','case', 'anno')])
            ...
            if balance_classes_total_tiles[i]:
                ...
                if use_n == 0: #If there are no slides in one class, revert to n of class with slides!
                ...
                print('Not balancing class totals for %s ... %d neg %d pos, use all.'
                      % (set_lab[i], nneg, npos))
                temp = pd.DataFrame(bal_dat.loc[:, ('fn', 'slide', 'case','anno')]
                                    ).reset_index(drop=True)
            temp['is_valid'] = i
            if i == 0:
                out = temp
