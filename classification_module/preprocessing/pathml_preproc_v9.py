...
file_list_pnfn = sys.argv[2] #Sampleinfo spreadsheet destination e.g. 'new_data_20220829_diagnosis_data.tsv'
job_id = int(sys.argv[3]) # pick file to use
tile_size = int(sys.argv[4])
helper_path = sys.argv[5] #'/ix/rbao/Projects/panCancer_HE/scripts/pancancer_he_classifier'
...
tile_dest = dest.joinpath('v%d' % version).joinpath('tiles').joinpath('%dpx' % tile_size)
time_start_str=time.strftime('%Y-%m-%d %H:%M:%S%p', time.localtime(start))

print("Uses: blank tile detection-> artifact detection -> tissue > %d%%, for final tile inclusion" % tissue_thresh)
...
tile_dest = dest.joinpath('v%d' % version).joinpath('tiles').joinpath('%dpx' % tile_size).joinpath(use_slide.split('.')[0])
...
if 'anno_path' in info.columns:
    ...
    if gj_fn.exists():
        use_anno = True
        print('annotation file found: will check overlaps',info.annotation.values[job_id])
...
if file_avail:    
    ...
    print('\t\tSave tiles at %dx%d pixel size (tissue only > %d%%), stride = %d' % (tile_size,
                                                                       tile_size,
                                                                       tissue_thresh,
                                                                       tile_stride))
    print('\t\tTotal tile estimate (without filtering blank tiles etc): %d' % tot_tiles_est)
                ...
                if (blur_detect > blur_thresh):
                    ...
                    if (per > tissue_thresh) :
                        ...
                        if use_anno:
                            ...
                            for iii,feat in enumerate(enum_obj): #Be aware that this structure can change considerably
                                if 'classification' in feat['properties'].keys():
                                    anno_type = feat['properties']['classification']['name'] 
                                else:
                                    anno_type = 'anno'                                    
                                print('Finding tiles in %s' % anno_type)
                                per_overlap = annoHelper.check_tile_overlap_feat(feat,[x,y],tile_size)
